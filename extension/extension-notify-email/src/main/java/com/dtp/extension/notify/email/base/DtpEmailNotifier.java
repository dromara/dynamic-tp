...
public class DtpEmailNotifier extends AbstractDtpNotifier {
    ...
    protected String buildAlarmContent(NotifyPlatform platform, NotifyItemEnum notifyItemEnum) {
        ...
        val capturedExecutor = alarmCtx.getCapturedExecutor();
        ...
        context.setVariable("corePoolSize", capturedExecutor.getCorePoolSize());
        context.setVariable("maximumPoolSize", capturedExecutor.getMaximumPoolSize());
        context.setVariable("poolSize", capturedExecutor.getPoolSize());
        context.setVariable("activeCount", capturedExecutor.getActiveCount());
        context.setVariable("largestPoolSize", capturedExecutor.getLargestPoolSize());
        context.setVariable("taskCount", capturedExecutor.getTaskCount());
        context.setVariable("completedTaskCount", capturedExecutor.getCompletedTaskCount());
        context.setVariable("waitingTaskCount", capturedExecutor.getQueue().size());
        context.setVariable("queueType", executor.getQueue().getClass().getSimpleName());
        context.setVariable("queueCapacity", getQueueCapacity(capturedExecutor));
        context.setVariable("queueSize", capturedExecutor.getQueue().size());
        context.setVariable("queueRemaining", capturedExecutor.getQueue().remainingCapacity());
        context.setVariable("rejectType", getRejectHandlerName(capturedExecutor));
